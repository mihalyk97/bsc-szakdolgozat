...
struct EmailTemplate {
    ...
    func createEmail<E>(templateName: String, context: E, app: Application) throws -> EventLoopFuture<Email> where E: Encodable {
        return try self.render(
            templateName: templateName,
            context: context,
            app: app).map { body in
                let email = Environment.get("SMTP_EMAIL") ?? ""
                return Email(from: EmailAddress(address: email, name: ApplicationInfo.appname),
